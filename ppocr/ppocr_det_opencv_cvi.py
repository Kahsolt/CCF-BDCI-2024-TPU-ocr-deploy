...
class PPOCRv2Det:
    ...
    def __init__(self, args):
        ...
        self.mean = np.asarray([0.485, 0.456, 0.406]).reshape((1, 1, 3)).astype('float32') * 255.0
        self.scale = np.asarray([1/0.229, 1/0.224, 1/0.225]).reshape((1, 1, 3)).astype('float32') * 1 / 255.0
        self.count = 0
        # postprocess
    ...
    def preprocess(self, img):
        # tmp_org_im = img.copy()
        # tmp_org_im = np.transpose(tmp_org_im, (2, 0, 1))
        h, w, _ = img.shape
        size_max = max(h, w)
        ...
        # cv2.imwrite("results/resize_img.jpg", img)
        # tmp_resize_im = img.copy()
        # tmp_resize_im = np.transpose(tmp_resize_im, (2, 0, 1))
        ...
        # np.savez("npz_data/det_padding_im.npz", **{'padding_im': np.expand_dims(padding_im,0)}, **{'resize_im': np.expand_dims(tmp_resize_im,0), **{'input_im': np.expand_dims(tmp_org_im,0)}})
        return padding_im, [h, w, resize_h, resize_w]

    def predict(self, tensor):
        input_data = {self.input_name: np.array(tensor, dtype=np.float32)}
        # outputs = self.net.process(self.graph_name, input_data)
        # np.savez("npz_data/det_input.npz", **input_data)
        outputs = model_inference(input_data, self.net)
        # np.savez("npz_data/det_output_cv181x.npz", **outputs)
        if "cvi" in self.net:
            return outputs['sigmoid_0.tmp_0_Sigmoid_f32']
        else:
            return outputs['sigmoid_0.tmp_0_Sigmoid']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cvimodel_det', type=str, default='./models/ch_PP-OCRv3_det.cvimodel', help='cvimodel path')
